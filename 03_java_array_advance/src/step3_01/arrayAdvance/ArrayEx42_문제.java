...
public class ArrayEx42_문제 {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			System.out.print("  ");						//0,0 번째 공란
			for (int i = 0; i < size; i++) {			
				System.out.print(i + " ");				//0번행 입력(0~9)
			}											
			System.out.println();						//행 +1
			for (int i = 0; i < size; i++) {			
				System.out.print(i + " ");				//각행별 0번 열에 (0~size값) 순차적 입력
				for (int j = 0; j < size; j++) {		
					System.out.print(omok[i][j] + " "); //size만큼 omok 배열값(각행의 각열) 가져옴
				}
				System.out.println();
			}
